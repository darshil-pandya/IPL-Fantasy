...
function loadLeague(): { byId: Map<string, LeagueP>; normToIds: Map<string, string[]> } {
  ...
  const pj = JSON.parse(
    fs.readFileSync(path.join(ROOT, "public/IPL-Fantasy/data/players.json"), "utf8"),
  ) as { players: LeagueP[] };
  const wj = JSON.parse(
    fs.readFileSync(path.join(ROOT, "public/IPL-Fantasy/data/waiver-pool.json"), "utf8"),
  ) as { players: LeagueP[] };
...
function main(): void {
  void (async () => {
    ...
    const outPublic = path.join(ROOT, "public/IPL-Fantasy/data/espnSquadNameToLeaguePlayerId.json");
